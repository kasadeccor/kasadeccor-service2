...
function checkLotes() {
  return new Promise(async function (resolve, reject) {
    ...
    Produtos.find({ USA_LOTE: true }).exec(function (err, docs) {
      for (let i in docs) {
        ...
        from tb_estoque e
        where e.relmovestq = ${docs[i].CODIGO} and (e.movto = 'S' or e.movto = 'SP' or e.movto = 'E')
        ...
        from tb_produtos p
        where p.codigo = ${docs[i].CODIGO} and
              ...
              if (result.length > 0) {
                ...
                if (lotes.length > 0) {
                  try {
                    ...
                    let save = await Produtos.findOneAndUpdate({ CODIGO: docs[i].CODIGO }, { lotes: lotes }, { upsert: true, new: true, setDefaultsOnInsert: true, useFindAndModify: false }).exec();
