...
function checkOrcamentos() {
  ...
  return new Promise(async function (resolve, reject) {
    ...
    // db = DATABASE
    db.query(`
   select p.ralped as PEDIDO, t.nome_cli as NOME_CLIENTE, p.nr_item_estoq as CODPRODUTO,
   ...
   and (t.situacao = '' OR t.situacao = ' ' OR t.situacao is null)
   and t.ULTMODIF_APP between cast('${moment().subtract(3, 'days').format('D.M.YYYY')}' as date) and cast('${moment().add(2, 'days').format('D.M.YYYY')}' as date)
   --and (t.DATA_EDICAO = cast('now' as date) or t.D_BAIXA = cast('now' as date))
          ...
          for (let i in result) {
            if (result[i].PEDIDO == prevPed) {
              ...
              try {
               ...
               // console.log("Deleted code: ", desc.PEDIDO);
              } catch (err) {
                console.log(err)
              }
...
module.exports = checkOrcamentos