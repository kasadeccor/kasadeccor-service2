...
const checkLotesOnDemand = require('./checkLotesOnDemand');
let options = {};
...
function checkProdutosDiario(erro, db) {
  //console.log('Chamou checkProdutosDiario ', moment().format('lllll'))
  return new Promise(async function (resolve, reject) {
    ...
    `, async function (err, result) {
        ...
        if (!err) {
          let ProdutosLote = []
          let totalupdt = 0
          //console.log('check prod daily: ', result.length)
        //  console.log(result.length, `${moment().subtract(60, 'minutes').format('DD.MM.YYYY HH:mm:ss.SSS')} `)
          //cast('now' as date)

          for (let i in result) {

           // console.log('check prod daily: ', result[i].CODIGO)
            ...
            var query = {
              ...
              options = { upsert: true, new: false, setDefaultsOnInsert: true, useFindAndModify: false };
            // Find the document
            try {
              //console.log(update)
              let save = await Produtos.findOneAndUpdate(query, update, options).exec();
              // console.log('BEFORE', {codigo: save.CODIGO ,disp: save.ESTOQUE_DISPONIVEL})
              // console.log('AFTER', {codigo: result[i].CODIGO ,disp: result[i].ESTOQUE_DISPONIVEL})

              //console.log({save: save.ESTOQUE_DISPONIVEL})
              //console.log({result: result[i].ESTOQUE_DISPONIVEL})
             // if (result[i].USA_LOTE == 'S' && (save.ESTOQUE_DISPONIVEL != result[i].ESTOQUE_DISPONIVEL)) {
              if (result[i].USA_LOTE == 'S' && (save.ESTOQUE_DISPONIVEL != result[i].ESTOQUE_DISPONIVEL)) {
                ProdutosLote.push({ ...result[i] })
              }
              totalupdt++
          ...
          if (ProdutosLote.length > 0) {
            checkLotesOnDemand(ProdutosLote)
          }
          //db.detach()
          resolve()
          //resolve()
...
module.exports = checkProdutosDiario