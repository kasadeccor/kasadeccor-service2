...
function checkPedidosDaily(erro, db) {
  return new Promise(async function (resolve, reject) {
   ...
   and t.ULTMODIF_APP > '${moment().subtract(2, 'days').format('DD.MM.YYYY HH:mm:ss.SSS')}'
   --and t.CODIGO = 81061
            ...
            for (let i in result) {
              if (result[i].PEDIDO == prevPed) {
                ...
                if (result[i].SITUACAO == 'PD') {
                 // console.log('Codigo: ', result[i].PEDIDO , ' SITUACAO: ', result[i].SITUACAO)
                ...
                if (findPed.length == 0 && (desc.status_pedido == 50 || desc.status_pedido == 0)) {
                  ...
                  try {
                    //console.log('Nao encontrou pedido: ', desc.PEDIDO, 'PERC: ', desc.DESC_PERC_PEDIDO)
                    ...
                    if (saved) {
                      try {
                        //console.log('Delete pedido da MOV: ', desc.PEDIDO)
