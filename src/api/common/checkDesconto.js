...
// })
let flag = false
function checkDesconto(erro, db) {
  //console.log('Chamou checkDesconto ', moment().format('lllll'))
  return new Promise(async function (resolve, reject) {
   ...
   and t.situacao = 'PD'
   and t.data_edicao = cast('now' as date)
   and ( cast(ROUND(((t.desc_ped / t.total_bruto_ped) * 100),2) as decimal(12,2)) > 5)
   order by p.ralped desc;
    `, async function (err, result) {

          if (!err) {
            if (result.length > 0) {
              // console.log('Quantidade de dados da query (DESCONTOS): ' + result.length)
                ...
                if (result[i].PEDIDO == prevPed) {
                  ...
                  if (findPed.length == 0) {
                    ...
                    try {
                      ...
                      push.header = "ALERTA! Desconto acima de 10%"
